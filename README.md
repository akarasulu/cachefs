...
- **Homebrew bottle from this repo:** `make brew` (alias `make tap`) builds a Homebrew bottle in `dist/bottles/` using the local formula. After uploading the `.tar.gz` and `.bottle.json` somewhere permanent, add the `bottle do` block that `brew bottle` outputs into `Formula/cachefs.rb` with the generated `sha256` entries. Once that block is present, users can `brew tap <user>/cachefs <repo-url>` and `brew install <user>/cachefs/cachefs` to pull the bottle automatically. macOS users still need to install/approve macFUSE (it is not auto-installed when bottling).
- **Using the published bottle:** Once the `bottle do` block is populated and pushed, end users can install with: `brew tap <user>/cachefs https://github.com/akarasulu/cachefs.git` then `brew install <user>/cachefs/cachefs`. This will pull the bottled binary for supported platforms. macOS users still must have the macFUSE runtime + developer headers installed/approved. Install it via DMG from the git releases here <https://github.com/macfuse/macfuse/releases> (developed and worked on 5.1.2)
